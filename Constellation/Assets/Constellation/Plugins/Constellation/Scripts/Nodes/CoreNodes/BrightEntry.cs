...
{
    public class BrightEntry : INode, IReceiver, IMirrorNode
    {
        private ISender sender;
        private Parameter parameter; // attributes are setted in the editor.
        public const string NAME = "BrightEntry"; //Setting the node name (need to be a const to be used in the factory without the node instantiated)

        public void Setup(INodeParameters _node)
        {
            var wordValue = new Ray();
            _node.AddOutput(true, "Any", "Output the entry received"); // setting a cold input
